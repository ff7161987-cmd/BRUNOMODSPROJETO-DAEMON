...
public:
    ...
    Canvas(JNIEnv *env, int width, int height, float density)
    {
        ...
        m_Rect = new Rect2(this->env);
        m_Typeface = new Typeface(this->env);
        m_CurrentTypeface = env->NewGlobalRef(m_Typeface->create("BrunoMods", (int)FontStyle::BOLD));
    }
    ...
    void drawText(const char *text, float X, float Y, float size = 13, Align align = Align::LEFT, long textColor = 0xFFFFFFFF, long outlineColor = 0x0);
