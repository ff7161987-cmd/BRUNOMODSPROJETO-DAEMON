...
MemoryPatch::MemoryPatch(uintptr_t absolute_address, 
  ...
  bool MemoryPatch::Restore() {
    ...
    return KittyMemory::memWrite(reinterpret_cast<void *>(_address), &_orig_code[0], _size) == Memory_Status2::SUCCESS;
  }

  bool MemoryPatch::Modify() {
    if (!isValid()) return false;
    return (KittyMemory::memWrite(reinterpret_cast<void *>(_address), &_patch_code[0], _size) == Memory_Status2::SUCCESS);
  }
