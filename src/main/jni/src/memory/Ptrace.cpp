...
bool Ptrace::ProtectAddr(void *addr, size_t length, int protection) {
...
Memory_Status Ptrace::memWrite(void *addr, const void *buffer, size_t len) {
    if (addr == NULL)
        return INV_ADDR;
...
Memory_Status Ptrace::memRead(void *buffer, const void *addr, size_t len) {
    if (addr == NULL)
        return INV_ADDR;
...
std::string Ptrace::read2HexStr(const void *addr, size_t len) {
    char temp[len];
    memset(temp, 0, len);
...
ProcMap Ptrace::getLibraryMap(const char *libraryName) {
    ProcMap retMap;
    char line[512] = {0};
...
uintptr_t Ptrace::getAbsoluteAddress(const char *libraryName, uintptr_t relativeAddr) {
