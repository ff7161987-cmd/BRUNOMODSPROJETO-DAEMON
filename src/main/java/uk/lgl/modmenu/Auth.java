...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    private WeakReference<Login> weakActivity;
    private ProgressDialog pDialog;
    private  String a = "a";
    public static String URL = "SITEAKI";
	...
	private native void contextM();
	
    @Override
    protected String doInBackground(String... strings) {
        if (!isInternetAvailable(getActivity())) {
            return "Sem acesso há internet";
        }
        try {
    ...
    protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity.getApplicationContext(), "Erro no Servidor!", 0).show();
			return;
        }
        ...
        try {
            ...
            if (!verify(decData, ack.get("Sign").toString(), puk)) {
				Toast.makeText(activity.getApplicationContext(), "Dados Incorretos!", 0).show();
                return;
            }
            ...
            if(data.get("Status").toString().equals("Success")) {
             ...
             try {
               ...
               try {            
			    Toast.makeText(activity.getApplicationContext(), "Conectado, Bem-vindo " + data.get("Dias") + " Divirta-se", 0).show();
				...
				Toast.makeText(activity.getApplicationContext(), "" + data.get("MessageString").toString() + "", 0).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
